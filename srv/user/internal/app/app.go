...
import (
	...
	"microx/srv/user/internal/app/factory"
)
...
func NewApplication() Application {
	...
	b := stan.NewBroker(
		...
		stan.DurableName(config.Broker.DurableName),
	...
	// Register Handler
	user.RegisterUserHandler(app.service.Server(), factory.GetUserHandler())
	micro.RegisterSubscriber(
		constant.TOPIC_USER_CREATED,
		app.service.Server(),
		factory.GetSubscriber().UserCreated(),
		server.SubscriberQueue(config.Broker.Queue),
	)
	capx.RegisterConsumer(constant.TOPIC_USER_CREATED, factory.GetSubscriber().CapxUserCreated())
